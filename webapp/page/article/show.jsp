...
<body>
	...
	<c:if test="${article.userName ne userName}">
        ...
        <c:if test="${article.userName eq userName }">
			<button  class="button" style="background-color: #4CAF50" onclick="window.location.href='/sharePlatform/article/show?articleID=${article.articleID}&showType=modify';"  >编辑文章</button>
		</c:if>
		                ...
		                <c:forEach items="${list}" var="m">
		                    <tr>
		                        ...
		                        <td style="text-align: center; width: 50px">
		                        	<c:if test="${buyFlag eq 'false' && userType eq 'user' && !article.userName eq userName}">
		                            	<input type="button" disabled=true onclick="window.location.href='/sharePlatform/means/download?meansID=${m.meansID}&meansName=${m.meansName}&articleID=${article.articleID}';" class="button" value="购买后可下载" style="background-color: #333;">
		                        	</c:if>
		                        	<c:if test="${buyFlag eq 'true' || userType eq 'admin' || article.userName eq userName}">
		                        		<input type="button" onclick="window.location.href='/sharePlatform/means/download?meansID=${m.meansID}&meansName=${m.meansName}&articleID=${article.articleID}';" class="button" value="下载" style="background-color: #333;">
		                        	</c:if>
		                        </td>
		                    </tr>
		                </c:forEach>
